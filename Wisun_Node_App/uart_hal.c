...
#define APP_USART0_TX_PORT gpioPortB
#define APP_USART0_TX_PIN 3

#define APP_USART0_RX_PORT gpioPortB
#define APP_USART0_RX_PIN 2

#define UART_TIMEOUT_US 100000 // Timeout duration in milliseconds
/*
** ============================================================================
...
volatile bool dataready=false;
 ...
 // Timer for UART timeout
volatile bool uart_timeout = false; // Flag to indicate timeout
...
** ============================================================================
*/
// UART timeout callback function pointer
static void (*uart_timeout_cb)(void) = NULL;

// Timer callback function
void uart_timeout_callback(RAIL_Handle_t railHandle)
{
    
  // Call the registered timeout callback
    if (uart_timeout_cb != NULL) {
        uart_timeout_cb();
    }
}
...
//  g_uart0_rx_length=UART_RX_BUF_LEN;
...
/* Register UART timeout callback */
void register_uart_timeout_cb(void (*callback)(void))
{
    uart_timeout_cb = callback;
}

/* Initialize the RAIL timer */
void UART_init_rail_timer(RAIL_Handle_t handle)
{
    railHandle = handle;
}
/******************************************************************************/
...
{
    uint32_t sent = 0;
  uint8_t * _buffer = (uint8_t *) p_data;
 ...
 *
 *****************************************************************************/
//uint8_t uart_debug_buff [2000] = {0};
...
void USART0_RX_IRQHandler(void)
{
  ...
  if (uart0->STATUS & USART_STATUS_RXDATAV) {
    ...
    r_buff.buffer[r_buff.head] = rxData;
    r_buff.head = (r_buff.head + 1) % RING_BUFFER_SIZE;

    // Restart the RAIL timer
    RAIL_SetTimer(railHandle, UART_TIMEOUT_US, RAIL_TIME_DELAY, uart_timeout_callback);
}
}
