...
#include "application/tun_app.h"
#include "application/mqtt.h"
#include "application/db.h"
...
static void wsbr_network_init(struct wsbr_ctxt *ctxt)
{
    ...
    ctxt->tun_client_fd=tun_initialise(ctxt->config.tun_dev);
    if (ctxt->tun_client_fd < 0) {
        FATAL(1, "Failed to initialize tun_client socket");
    }
    capture_register_netfd(ctxt->tun_client_fd);
}
...
static void wsbr_fds_init(struct wsbr_ctxt *ctxt)
{
    ...
    //tun client
    ctxt->fds[POLLFD_TUN_CLIENT].fd = ctxt->tun_client_fd;
    ctxt->fds[POLLFD_TUN_CLIENT].events = POLLIN;
}
...
static void wsbr_poll(struct wsbr_ctxt *ctxt)
{
    ...
    if (ctxt->fds[POLLFD_TUN].revents & POLLIN)
        wsbr_tun_read(ctxt);
    if (ctxt->fds[POLLFD_TUN_CLIENT].revents & POLLIN)
        tun_app_read(ctxt);
