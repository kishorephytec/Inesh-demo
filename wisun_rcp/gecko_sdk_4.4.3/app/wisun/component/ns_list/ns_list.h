 ...
 * by optionally-in line functions. The macros do not evaluate any arguments more
 ...
 *
 * This union stores the real list head and encodes as compile-time type
 * information the offset of the link pointer and the type of the entry.
 *
 * Note that type information is compiler-dependent, which means
 ...
 *
 * To support C++03 compilers that cannot return properly-typed
 * pointers, such as IAR 7, use NS_LIST_TYPECOERCE to force the type.
 * ~~~
 ...
 * ~~~
 * If you need to use a list type in multiple places, e.g., as a function
...
/** \brief Initializer for an entry's link member.
 *
 * This initializer is not required by the library, but a user may want an
 * initializer to include in their own entry initializer. See
...
/** \hideinitializer \brief Initialize an entry's list link.
 *
 * This "initializes" an unattached entry's link by filling the fields with
 ...
 * \param field The name of the link member to initialize
 */
#define ns_list_link_init(entry, field) ns_list_link_init_(&(entry)->field)

/** \hideinitializer \brief Initialize a list
 *
 * Initialize a list head before use. A list head must be initialized using this
 * function or one of the NS_LIST_INIT()-type macros before use. A zero-initialized
...
/** \brief Initializer for an empty list.
 *
 * Usage in an enclosing initializer:
 * ~~~
...
/** \brief Name and initializer for an empty list
 *
 * Usage:
 * ~~~
 ...
 * ~~~
 * Also useful with designated initializers:
 * ~~~
...
/** \brief Define a list, and initialize to empty.
 *
 * Usage:
 * ~~~
...
NS_LIST_FN void *ns_list_get_previous_(const ns_list_t *list, ns_list_offset_t offset, const void *current)
{
  ...
  // pointer that points to us - i.e., &head->first_entry OR &{prev}->next.
